...
const esquema = z.object({
  fechaAplicacion: z
    ...
    .refine((f) => f <= hoyIso(), 'La fecha de aplicación no puede ser futura.'),
  avesVacunadas: z
    .string()
    .refine((v) => v === '' || Number(v) > 0, 'La cantidad debe ser mayor que cero.'),
  observaciones: z.string(),
});
type DatosFormulario = z.infer<typeof esquema>;

export function CompletarTareaDialog({
  tarea,
  abierto,
  alCerrar,
}: {
  tarea: TareaVacunacionResumen | null;
  abierto: boolean;
  alCerrar: () => void;
}) {
  ...
  const guardar = useMutation({
    mutationFn: (datos: DatosFormulario) =>
      completarTareaVacunacion(tarea!.id, {
    ...
    <Dialog open={abierto} onClose={alCerrar}>
      ...
      <DialogContent>
        <TextField
          ...
          error={Boolean(errors.fechaAplicacion)}
          helperText={errors.fechaAplicacion?.message}
        ...
        <TextField
          ...
          {...register('avesVacunadas')}
          error={Boolean(errors.avesVacunadas)}
          helperText={errors.avesVacunadas?.message ?? 'Dejalo vacío si se vacunó todo el galpón.'}
          inputMode="numeric"
          fullWidth
          margin="dense"
        />
      ...
      <DialogActions>
        ...
        <Button
          onClick={() => void handleSubmit((datos) => guardar.mutate(datos))()}
          disabled={!online || guardar.isPending || !tarea}
        >
          Guardar
        </Button>
      </DialogActions>
