...
function baseFetchAvicola(reglas: Record<string, Response | Response[]>) {
  return baseFetchAvicolaConFuncionalidades(['Granjas', 'Galpones', 'ProduccionHuevos'], reglas);
}
  ...
  return render(
    <QueryClientProvider client={queryClient}>
      <MemoryRouter initialEntries={[rutaInicial]}>
        <AuthProvider>
          <Routes>
            <Route path="/avicola/galpones" element={<GalponesPage />} />
          </Routes>
        </AuthProvider>
      </MemoryRouter>
    </QueryClientProvider>,
...
describe('GalponesPage', () => {
  ...
  test('muestra granja, tarjeta y eficiencia', async () => {
    baseFetchAvicola({
      ...
      'GET /api/galpones/ga1/eficiencia': respuesta(200, {
        ...
        desde: '2026-08-19',
        hasta: '2026-08-19',
        dias: [{ fecha: '2026-08-19', eficiencia: 62.5, bajoUmbral: true }],
      }),
    });
    ...
    expect(screen.getByRole('link', { name: /abrir galpón 1/i })).toHaveAttribute(
      'href',
      '/avicola/galpones/ga1',
    );
  });
  test('estado vacío invita a crear', async () => {
    baseFetchAvicola({
      'GET /api/granjas': respuesta(200, [granja]),
  ...
  test('renombra la granja', async () => {
    ...
    const fetchMock = baseFetchAvicola({
      'GET /api/granjas': [
        respuesta(200, [granja]),
        respuesta(200, [{ ...granja, nombre: 'Granja Sur' }]),
      ],
      'GET /api/granjas/gr1/galpones': respuesta(200, []),
  ...
  test('sin funcionalidad Granjas oculta renombrar y alta', async () => {
    baseFetchAvicolaConFuncionalidades(
      ['Galpones', 'ProduccionHuevos'],
      {
        'GET /api/granjas': respuesta(200, [granja]),
        'GET /api/granjas/gr1/galpones': respuesta(200, []),
      },
