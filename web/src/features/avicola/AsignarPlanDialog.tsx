...
import {
  CLAVE_NOTIFICACION_VACUNACION,
  CLAVE_PROGRAMAS_VACUNACION,
  CLAVE_TAREAS_VACUNACION,
} from './constantes';
...
export function AsignarPlanDialog({
  ...
  const programas = useQuery({
    queryKey: CLAVE_PROGRAMAS_VACUNACION,
    queryFn: () => listarProgramasVacunacion(),
    enabled: abierto,
  });
    ...
    <Dialog open={abierto} onClose={alCerrar}>
      ...
      <DialogContent>
        ...
        <Alert severity="warning" sx={{ mt: 2 }}>
          Si el galpón ya tiene un plan, las pendientes del plan anterior se desactivan. Las
          completadas y canceladas se conservan como historial.
        </Alert>
        {asignar.isError && (
          <Alert severity="error" sx={{ mt: 1 }}>
            {asignar.error instanceof ApiError
      ...
      <DialogActions>
        ...
        <Button
          ...
          disabled={!online || asignar.isPending || !programaId}
        >
          Asignar
        </Button>
      </DialogActions>
