// <auto-generated />
using System;
using Icarus.GestionAvicola.Infrastructure.Persistencia;
using Microsoft.EntityFrameworkCore;
using Microsoft.EntityFrameworkCore.Infrastructure;
using Microsoft.EntityFrameworkCore.Metadata;
using Microsoft.EntityFrameworkCore.Migrations;
using Microsoft.EntityFrameworkCore.Storage.ValueConversion;

#nullable disable

namespace Icarus.GestionAvicola.Infrastructure.Migrations
{
    [DbContext(typeof(GestionAvicolaDbContext))]
    [Migration("20260914144923_TrazabilidadGranjaAutorYFolio")]
    partial class TrazabilidadGranjaAutorYFolio
    {
        /// <inheritdoc />
        protected override void BuildTargetModel(ModelBuilder modelBuilder)
        {
#pragma warning disable 612, 618
            modelBuilder
                .HasDefaultSchema("gestion_avicola")
                .HasAnnotation("ProductVersion", "10.0.11")
                .HasAnnotation("Relational:MaxIdentifierLength", 128);

            SqlServerModelBuilderExtensions.UseIdentityColumns(modelBuilder);

            modelBuilder.Entity("Icarus.GestionAvicola.Domain.AjusteCreditoHuevo", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier");

                    b.Property<Guid>("ActorId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<Guid>("ClienteId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<DateTime>("CreadoEnUtc")
                        .HasColumnType("datetime2");

                    b.Property<Guid>("DespachoHuevoId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<decimal>("Monto")
                        .HasColumnType("decimal(18,2)");

                    b.Property<string>("Motivo")
                        .IsRequired()
                        .HasMaxLength(500)
                        .HasColumnType("nvarchar(500)");

                    b.Property<Guid>("PublicacionCorrectivaId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<Guid>("PublicacionErroneaId")
                        .HasColumnType("uniqueidentifier");

                    b.HasKey("Id");

                    b.HasIndex("ClienteId");

                    b.HasIndex("DespachoHuevoId");

                    b.ToTable("ajustes_credito_huevo", "gestion_avicola", t =>
                        {
                            t.HasCheckConstraint("CK_ajustes_credito_huevo_monto", "[Monto] <> 0");
                        });
                });

            modelBuilder.Entity("Icarus.GestionAvicola.Domain.DespachoHuevo", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier");

                    b.Property<Guid>("ClienteId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<Guid>("CreadoPor")
                        .HasColumnType("uniqueidentifier");

                    b.Property<Guid?>("CreadoPorTrabajadorId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<bool>("EstaActivo")
                        .HasColumnType("bit");

                    b.Property<int>("Estado")
                        .HasColumnType("int");

                    b.Property<DateOnly?>("FechaDespacho")
                        .HasColumnType("date");

                    b.Property<DateOnly?>("FechaRecepcion")
                        .HasColumnType("date");

                    b.Property<Guid>("GranjaId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<int>("Numero")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasDefaultValueSql("NEXT VALUE FOR gestion_avicola.secuencia_despachos_huevo");

                    b.Property<byte[]>("Version")
                        .IsConcurrencyToken()
                        .ValueGeneratedOnAddOrUpdate()
                        .HasColumnType("rowversion");

                    b.HasKey("Id");

                    b.HasIndex("CreadoPorTrabajadorId");

                    b.HasIndex("Numero")
                        .IsUnique();

                    b.HasIndex("ClienteId", "FechaDespacho");

                    b.HasIndex("ClienteId", "GranjaId");

                    b.HasIndex("Estado", "ClienteId", "FechaDespacho");

                    b.ToTable("despachos_huevo", "gestion_avicola");
                });

            modelBuilder.Entity("Icarus.GestionAvicola.Domain.DetalleDespachoHuevo", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier");

                    b.Property<int>("CantidadAmarras")
                        .HasColumnType("int");

                    b.Property<Guid>("DespachoHuevoId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<decimal?>("PrecioUnitarioCongelado")
                        .HasColumnType("decimal(10,4)");

                    b.Property<Guid?>("PublicacionPrecioHuevoId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<int>("Tamano")
                        .HasColumnType("int");

                    b.Property<int>("UnidadesSueltas")
                        .HasColumnType("int");

                    b.HasKey("Id");

                    b.HasIndex("DespachoHuevoId", "Tamano")
                        .IsUnique();

                    b.ToTable("detalles_despacho_huevo", "gestion_avicola");
                });

            modelBuilder.Entity("Icarus.GestionAvicola.Domain.DetalleEntregaPedidoAlimento", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier");

                    b.Property<int>("CantidadEntregada")
                        .HasColumnType("int");

                    b.Property<Guid>("EntregaPedidoAlimentoId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<int>("Equivalentes40Kg")
                        .HasColumnType("int");

                    b.Property<int>("Presentacion")
                        .HasColumnType("int");

                    b.Property<int>("TipoAlimento")
                        .HasColumnType("int");

                    b.HasKey("Id");

                    b.HasIndex("EntregaPedidoAlimentoId", "TipoAlimento")
                        .IsUnique();

                    b.ToTable("detalles_entregas_pedidos_alimentos", "gestion_avicola", t =>
                        {
                            t.HasCheckConstraint("CK_detalles_entregas_cantidad", "[CantidadEntregada] >= 0");
                        });
                });

            modelBuilder.Entity("Icarus.GestionAvicola.Domain.DetallePedidoAlimento", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier");

                    b.Property<int>("CantidadSolicitada")
                        .HasColumnType("int");

                    b.Property<int>("Equivalentes40Kg")
                        .HasColumnType("int");

                    b.Property<Guid?>("NotificacionPreciosAlimentosId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<Guid>("PedidoAlimentoId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<decimal?>("PrecioFinalPor40Kg")
                        .HasColumnType("decimal(10,2)");

                    b.Property<int>("Presentacion")
                        .HasColumnType("int");

                    b.Property<decimal?>("SubtotalSolicitado")
                        .HasColumnType("decimal(18,2)");

                    b.Property<int>("TipoAlimento")
                        .HasColumnType("int");

                    b.HasKey("Id");

                    b.HasIndex("PedidoAlimentoId", "TipoAlimento")
                        .IsUnique();

                    b.ToTable("detalles_pedidos_alimentos", "gestion_avicola", t =>
                        {
                            t.HasCheckConstraint("CK_detalles_pedidos_cantidad", "[CantidadSolicitada] > 0");

                            t.HasCheckConstraint("CK_detalles_pedidos_precio", "[PrecioFinalPor40Kg] IS NULL OR [PrecioFinalPor40Kg] > 0");
                        });
                });

            modelBuilder.Entity("Icarus.GestionAvicola.Domain.DetallePrecioAlimento", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier");

                    b.Property<int?>("EdadDesdeDias")
                        .HasColumnType("int");

                    b.Property<int?>("EdadHastaDias")
                        .HasColumnType("int");

                    b.Property<Guid>("NotificacionPreciosAlimentosId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<decimal?>("PrecioActualDocumento")
                        .HasColumnType("decimal(10,2)");

                    b.Property<decimal>("PrecioFinalPor40Kg")
                        .HasColumnType("decimal(10,2)");

                    b.Property<int>("Presentacion")
                        .HasColumnType("int");

                    b.Property<int>("TipoAlimento")
                        .HasColumnType("int");

                    b.HasKey("Id");

                    b.HasIndex("NotificacionPreciosAlimentosId", "TipoAlimento", "Presentacion")
                        .IsUnique();

                    b.ToTable("detalles_precio_alimento", "gestion_avicola", t =>
                        {
                            t.HasCheckConstraint("CK_detalles_precio_final", "[PrecioFinalPor40Kg] > 0");
                        });
                });

            modelBuilder.Entity("Icarus.GestionAvicola.Domain.DetallePrecioHuevo", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier");

                    b.Property<decimal?>("PrecioActualDocumento")
                        .HasColumnType("decimal(10,4)");

                    b.Property<decimal>("PrecioAlProductor")
                        .HasColumnType("decimal(10,4)");

                    b.Property<Guid>("PublicacionPrecioHuevoId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<int>("Tamano")
                        .HasColumnType("int");

                    b.HasKey("Id");

                    b.HasIndex("PublicacionPrecioHuevoId", "Tamano")
                        .IsUnique();

                    b.ToTable("detalles_precio_huevo", "gestion_avicola", t =>
                        {
                            t.HasCheckConstraint("CK_detalles_precio_huevo_productor", "[PrecioAlProductor] > 0");
                        });
                });

            modelBuilder.Entity("Icarus.GestionAvicola.Domain.DetalleRecepcionPedidoAlimento", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier");

                    b.Property<int>("CantidadRecibida")
                        .HasColumnType("int");

                    b.Property<int>("Equivalentes40Kg")
                        .HasColumnType("int");

                    b.Property<int>("Presentacion")
                        .HasColumnType("int");

                    b.Property<Guid>("RecepcionPedidoAlimentoId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<int>("TipoAlimento")
                        .HasColumnType("int");

                    b.HasKey("Id");

                    b.HasIndex("RecepcionPedidoAlimentoId", "TipoAlimento")
                        .IsUnique();

                    b.ToTable("detalles_recepciones_pedidos_alimentos", "gestion_avicola", t =>
                        {
                            t.HasCheckConstraint("CK_detalles_recepciones_cantidad", "[CantidadRecibida] >= 0");
                        });
                });

            modelBuilder.Entity("Icarus.GestionAvicola.Domain.DocumentoDespachoHuevo", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier");

                    b.Property<Guid>("ClaveOriginal")
                        .HasColumnType("uniqueidentifier");

                    b.Property<Guid>("ClaveVista")
                        .HasColumnType("uniqueidentifier");

                    b.Property<Guid?>("DespachoHuevoId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<DateTime>("FechaUtc")
                        .HasColumnType("datetime2");

                    b.Property<string>("HashSha256")
                        .IsRequired()
                        .HasMaxLength(64)
                        .HasColumnType("nvarchar(64)");

                    b.Property<string>("Mime")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.Property<string>("NombreSeguro")
                        .IsRequired()
                        .HasMaxLength(200)
                        .HasColumnType("nvarchar(200)");

                    b.Property<long>("TamanoBytes")
                        .HasColumnType("bigint");

                    b.Property<long>("TamanoVistaBytes")
                        .HasColumnType("bigint");

                    b.HasKey("Id");

                    b.HasIndex("DespachoHuevoId")
                        .IsUnique()
                        .HasFilter("[DespachoHuevoId] IS NOT NULL");

                    b.ToTable("documentos_despacho_huevo", "gestion_avicola");
                });

            modelBuilder.Entity("Icarus.GestionAvicola.Domain.DocumentoNotaEntrega", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier");

                    b.Property<Guid>("ClaveOriginal")
                        .HasColumnType("uniqueidentifier");

                    b.Property<Guid>("ClaveVista")
                        .HasColumnType("uniqueidentifier");

                    b.Property<Guid>("EntregaPedidoAlimentoId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<DateTime>("FechaUtc")
                        .HasColumnType("datetime2");

                    b.Property<string>("HashSha256")
                        .IsRequired()
                        .HasMaxLength(64)
                        .HasColumnType("nvarchar(64)");

                    b.Property<string>("Mime")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<string>("NombreSeguro")
                        .IsRequired()
                        .HasMaxLength(200)
                        .HasColumnType("nvarchar(200)");

                    b.Property<long>("TamanoBytes")
                        .HasColumnType("bigint");

                    b.Property<long>("TamanoVistaBytes")
                        .HasColumnType("bigint");

                    b.HasKey("Id");

                    b.HasIndex("EntregaPedidoAlimentoId");

                    b.ToTable("documentos_nota_entrega", "gestion_avicola");
                });

            modelBuilder.Entity("Icarus.GestionAvicola.Domain.EntregaPedidoAlimento", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier");

                    b.Property<DateOnly>("FechaDespacho")
                        .HasColumnType("date");

                    b.Property<DateOnly>("FechaNota")
                        .HasColumnType("date");

                    b.Property<string>("NumeroNota")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.Property<Guid>("PedidoAlimentoId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<decimal?>("TotalNetoInformado")
                        .HasColumnType("decimal(18,2)");

                    b.HasKey("Id");

                    b.HasIndex("PedidoAlimentoId")
                        .IsUnique();

                    b.ToTable("entregas_pedidos_alimentos", "gestion_avicola");
                });

            modelBuilder.Entity("Icarus.GestionAvicola.Domain.Galpon", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier");

                    b.Property<int>("CapacidadMaxima")
                        .HasColumnType("int");

                    b.Property<Guid>("ClienteId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<string>("Descripcion")
                        .HasMaxLength(500)
                        .HasColumnType("nvarchar(500)");

                    b.Property<bool>("EstaActivo")
                        .HasColumnType("bit");

                    b.Property<DateOnly>("FechaNacimientoLote")
                        .HasColumnType("date");

                    b.Property<int>("GallinasActuales")
                        .HasColumnType("int");

                    b.Property<Guid>("GranjaId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<string>("Numero")
                        .IsRequired()
                        .HasMaxLength(10)
                        .HasColumnType("nvarchar(10)");

                    b.HasKey("Id");

                    b.HasIndex("ClienteId");

                    b.HasIndex("GranjaId", "Numero")
                        .IsUnique();

                    b.ToTable("galpones", "gestion_avicola", t =>
                        {
                            t.HasCheckConstraint("CK_galpones_capacidad", "[CapacidadMaxima] > 0");

                            t.HasCheckConstraint("CK_galpones_inventario", "[GallinasActuales] >= 0 AND [GallinasActuales] <= [CapacidadMaxima]");
                        });
                });

            modelBuilder.Entity("Icarus.GestionAvicola.Domain.Granja", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier");

                    b.Property<Guid>("ClienteId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<bool>("EstaActivo")
                        .HasColumnType("bit");

                    b.Property<string>("Nombre")
                        .IsRequired()
                        .HasMaxLength(200)
                        .HasColumnType("nvarchar(200)");

                    b.HasKey("Id");

                    b.HasIndex("ClienteId")
                        .IsUnique()
                        .HasFilter("[EstaActivo] = 1");

                    b.HasIndex("ClienteId", "Nombre")
                        .IsUnique();

                    b.ToTable("granjas", "gestion_avicola");
                });

            modelBuilder.Entity("Icarus.GestionAvicola.Domain.ItemPlanVacunacion", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier");

                    b.Property<int>("EdadDia")
                        .HasColumnType("int");

                    b.Property<bool>("EstaActivo")
                        .HasColumnType("bit");

                    b.Property<DateOnly?>("Fecha")
                        .HasColumnType("date");

                    b.Property<string>("ModoAplicacion")
                        .HasMaxLength(500)
                        .HasColumnType("nvarchar(500)");

                    b.Property<string>("Observaciones")
                        .HasMaxLength(1000)
                        .HasColumnType("nvarchar(1000)");

                    b.Property<Guid>("ProgramaVacunacionId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<string>("Vacuna")
                        .IsRequired()
                        .HasMaxLength(200)
                        .HasColumnType("nvarchar(200)");

                    b.HasKey("Id");

                    b.HasIndex("ProgramaVacunacionId");

                    b.ToTable("programas_vacunacion_items", "gestion_avicola", t =>
                        {
                            t.HasCheckConstraint("CK_programas_vacunacion_items_edad", "[EdadDia] > 0");
                        });
                });

            modelBuilder.Entity("Icarus.GestionAvicola.Domain.NotificacionInterna", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier");

                    b.Property<Guid?>("ClienteId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<DateTime?>("FechaLeidaUtc")
                        .HasColumnType("datetime2");

                    b.Property<DateTime>("FechaUtc")
                        .HasColumnType("datetime2");

                    b.Property<bool>("Leida")
                        .HasColumnType("bit");

                    b.Property<Guid?>("LeidaPor")
                        .HasColumnType("uniqueidentifier");

                    b.Property<string>("Meta")
                        .HasMaxLength(500)
                        .HasColumnType("nvarchar(500)");

                    b.Property<Guid>("PedidoId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<int>("Tipo")
                        .HasColumnType("int");

                    b.HasKey("Id");

                    b.HasIndex("PedidoId");

                    b.HasIndex("ClienteId", "FechaUtc");

                    b.ToTable("notificaciones_internas", "gestion_avicola");
                });

            modelBuilder.Entity("Icarus.GestionAvicola.Domain.NotificacionInternaDespachoHuevo", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier");

                    b.Property<Guid?>("ClienteId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<Guid?>("DespachoHuevoId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<DateTime?>("FechaLeidaUtc")
                        .HasColumnType("datetime2");

                    b.Property<DateTime>("FechaUtc")
                        .HasColumnType("datetime2");

                    b.Property<bool>("Leida")
                        .HasColumnType("bit");

                    b.Property<Guid?>("LeidaPor")
                        .HasColumnType("uniqueidentifier");

                    b.Property<string>("Meta")
                        .HasMaxLength(500)
                        .HasColumnType("nvarchar(500)");

                    b.Property<int>("Tipo")
                        .HasColumnType("int");

                    b.HasKey("Id");

                    b.HasIndex("DespachoHuevoId");

                    b.HasIndex("ClienteId", "FechaUtc");

                    b.ToTable("notificaciones_internas_despacho_huevo", "gestion_avicola");
                });

            modelBuilder.Entity("Icarus.GestionAvicola.Domain.NotificacionPreciosAlimentos", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier");

                    b.Property<decimal>("AporteCaisy")
                        .HasColumnType("decimal(10,2)");

                    b.Property<Guid?>("DocumentoOriginalId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<bool>("EstaActivo")
                        .HasColumnType("bit");

                    b.Property<int>("Estado")
                        .HasColumnType("int");

                    b.Property<DateOnly>("FechaDocumento")
                        .HasColumnType("date");

                    b.Property<decimal>("Fondo")
                        .HasColumnType("decimal(10,2)");

                    b.Property<decimal>("Servicios")
                        .HasColumnType("decimal(10,2)");

                    b.Property<byte[]>("Version")
                        .IsConcurrencyToken()
                        .ValueGeneratedOnAddOrUpdate()
                        .HasColumnType("rowversion");

                    b.Property<DateOnly>("VigenteDesde")
                        .HasColumnType("date");

                    b.HasKey("Id");

                    b.HasIndex("VigenteDesde")
                        .IsUnique()
                        .HasFilter("[Estado] = 1 AND [EstaActivo] = 1");

                    b.ToTable("notificaciones_precios_alimentos", "gestion_avicola", t =>
                        {
                            t.HasCheckConstraint("CK_notificaciones_precios_aportes", "[AporteCaisy] > 0 AND [Fondo] > 0 AND [Servicios] > 0");
                        });
                });

            modelBuilder.Entity("Icarus.GestionAvicola.Domain.PedidoAlimento", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier");

                    b.Property<Guid>("ClienteId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<Guid>("CreadoPor")
                        .HasColumnType("uniqueidentifier");

                    b.Property<Guid?>("CreadoPorTrabajadorId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<bool>("EstaActivo")
                        .HasColumnType("bit");

                    b.Property<int>("Estado")
                        .HasColumnType("int");

                    b.Property<DateOnly?>("FechaEntregaEstimada")
                        .HasColumnType("date");

                    b.Property<DateOnly?>("FechaPedido")
                        .HasColumnType("date");

                    b.Property<Guid>("GranjaId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<int>("Numero")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasDefaultValueSql("NEXT VALUE FOR gestion_avicola.secuencia_pedidos_alimento");

                    b.Property<byte[]>("Version")
                        .IsConcurrencyToken()
                        .ValueGeneratedOnAddOrUpdate()
                        .HasColumnType("rowversion");

                    b.HasKey("Id");

                    b.HasIndex("CreadoPorTrabajadorId");

                    b.HasIndex("Numero")
                        .IsUnique();

                    b.HasIndex("ClienteId", "FechaPedido");

                    b.HasIndex("ClienteId", "GranjaId");

                    b.HasIndex("Estado", "ClienteId", "FechaPedido");

                    b.ToTable("pedidos_alimentos", "gestion_avicola");
                });

            modelBuilder.Entity("Icarus.GestionAvicola.Domain.ProgramaVacunacion", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier");

                    b.Property<int>("CantidadAves")
                        .HasColumnType("int");

                    b.Property<bool>("EstaActivo")
                        .HasColumnType("bit");

                    b.Property<DateOnly?>("FechaEmision")
                        .HasColumnType("date");

                    b.Property<string>("Nombre")
                        .IsRequired()
                        .HasMaxLength(200)
                        .HasColumnType("nvarchar(200)");

                    b.Property<string>("Observaciones")
                        .HasMaxLength(1000)
                        .HasColumnType("nvarchar(1000)");

                    b.HasKey("Id");

                    b.HasIndex("Nombre")
                        .IsUnique();

                    b.ToTable("programas_vacunacion", "gestion_avicola", t =>
                        {
                            t.HasCheckConstraint("CK_programas_vacunacion_cantidad_aves", "[CantidadAves] > 0");
                        });
                });

            modelBuilder.Entity("Icarus.GestionAvicola.Domain.PublicacionPrecioHuevo", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier");

                    b.Property<Guid?>("DocumentoOriginalId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<bool>("EstaActivo")
                        .HasColumnType("bit");

                    b.Property<int>("Estado")
                        .HasColumnType("int");

                    b.Property<DateOnly>("FechaNotificacion")
                        .HasColumnType("date");

                    b.Property<DateOnly>("FechaVigencia")
                        .HasColumnType("date");

                    b.Property<string>("Motivo")
                        .HasMaxLength(500)
                        .HasColumnType("nvarchar(500)");

                    b.Property<Guid?>("PublicacionCorrectivaId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<decimal>("Servicio")
                        .HasColumnType("decimal(10,4)");

                    b.Property<byte[]>("Version")
                        .IsConcurrencyToken()
                        .ValueGeneratedOnAddOrUpdate()
                        .HasColumnType("rowversion");

                    b.HasKey("Id");

                    b.HasIndex("FechaVigencia")
                        .IsUnique()
                        .HasFilter("[Estado] = 1 AND [EstaActivo] = 1");

                    b.ToTable("publicaciones_precios_huevo", "gestion_avicola", t =>
                        {
                            t.HasCheckConstraint("CK_publicaciones_precios_huevo_servicio", "[Servicio] > 0");
                        });
                });

            modelBuilder.Entity("Icarus.GestionAvicola.Domain.RecepcionPedidoAlimento", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier");

                    b.Property<string>("DiferenciasJson")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<DateOnly>("FechaRecepcion")
                        .HasColumnType("date");

                    b.Property<Guid>("PedidoAlimentoId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<decimal>("TotalRecibido")
                        .HasColumnType("decimal(18,2)");

                    b.HasKey("Id");

                    b.HasIndex("PedidoAlimentoId")
                        .IsUnique();

                    b.ToTable("recepciones_pedidos_alimentos", "gestion_avicola");
                });

            modelBuilder.Entity("Icarus.GestionAvicola.Domain.RegistroMortalidad", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier");

                    b.Property<int>("CantidadMuertas")
                        .HasColumnType("int");

                    b.Property<Guid>("ClienteId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<bool>("EstaActivo")
                        .HasColumnType("bit");

                    b.Property<DateOnly>("Fecha")
                        .HasColumnType("date");

                    b.Property<int>("GallinasVivas")
                        .HasColumnType("int");

                    b.Property<Guid>("GalponId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<TimeOnly>("Hora")
                        .HasColumnType("time");

                    b.Property<Guid?>("IdempotencyKey")
                        .HasColumnType("uniqueidentifier");

                    b.HasKey("Id");

                    b.HasIndex("ClienteId");

                    b.HasIndex("IdempotencyKey")
                        .IsUnique()
                        .HasFilter("[IdempotencyKey] IS NOT NULL");

                    b.HasIndex("GalponId", "Fecha");

                    b.ToTable("registros_mortalidad", "gestion_avicola", t =>
                        {
                            t.HasCheckConstraint("CK_registros_mortalidad_cantidad", "[CantidadMuertas] > 0");
                        });
                });

            modelBuilder.Entity("Icarus.GestionAvicola.Domain.RegistroProduccion", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier");

                    b.Property<int>("CantidadMaples")
                        .HasColumnType("int");

                    b.Property<Guid>("ClienteId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<bool>("EstaActivo")
                        .HasColumnType("bit");

                    b.Property<DateOnly>("Fecha")
                        .HasColumnType("date");

                    b.Property<int>("GallinasVivas")
                        .HasColumnType("int");

                    b.Property<Guid>("GalponId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<TimeOnly>("Hora")
                        .HasColumnType("time");

                    b.Property<Guid?>("IdempotencyKey")
                        .HasColumnType("uniqueidentifier");

                    b.Property<int>("MaplesDescarte")
                        .HasColumnType("int");

                    b.Property<int>("UnidadesDescarte")
                        .HasColumnType("int");

                    b.Property<int>("UnidadesIncompletas")
                        .HasColumnType("int");

                    b.HasKey("Id");

                    b.HasIndex("ClienteId");

                    b.HasIndex("IdempotencyKey")
                        .IsUnique()
                        .HasFilter("[IdempotencyKey] IS NOT NULL");

                    b.HasIndex("GalponId", "Fecha");

                    b.ToTable("registros_produccion", "gestion_avicola", t =>
                        {
                            t.HasCheckConstraint("CK_registros_produccion_maples", "[CantidadMaples] >= 0 AND [MaplesDescarte] >= 0");

                            t.HasCheckConstraint("CK_registros_produccion_sueltos", "[UnidadesIncompletas] >= 0 AND [UnidadesIncompletas] < 30 AND [UnidadesDescarte] >= 0 AND [UnidadesDescarte] < 30");
                        });
                });

            modelBuilder.Entity("Icarus.GestionAvicola.Domain.TareaVacunacion", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier");

                    b.Property<int?>("AvesVacunadas")
                        .HasColumnType("int");

                    b.Property<Guid>("ClienteId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<Guid?>("CompletadaPor")
                        .HasColumnType("uniqueidentifier");

                    b.Property<int>("EdadDia")
                        .HasColumnType("int");

                    b.Property<bool>("EstaActivo")
                        .HasColumnType("bit");

                    b.Property<string>("Estado")
                        .IsRequired()
                        .HasMaxLength(20)
                        .HasColumnType("nvarchar(20)");

                    b.Property<DateOnly?>("FechaAplicacion")
                        .HasColumnType("date");

                    b.Property<DateOnly>("FechaProgramada")
                        .HasColumnType("date");

                    b.Property<Guid>("GalponId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<Guid>("ItemPlanVacunacionId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<string>("ModoAplicacion")
                        .HasMaxLength(500)
                        .HasColumnType("nvarchar(500)");

                    b.Property<string>("MotivoCancelacion")
                        .HasMaxLength(500)
                        .HasColumnType("nvarchar(500)");

                    b.Property<string>("ObservacionesAplicacion")
                        .HasMaxLength(1000)
                        .HasColumnType("nvarchar(1000)");

                    b.Property<string>("ObservacionesProgramadas")
                        .HasMaxLength(1000)
                        .HasColumnType("nvarchar(1000)");

                    b.Property<Guid>("ProgramaVacunacionId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<string>("Vacuna")
                        .IsRequired()
                        .HasMaxLength(200)
                        .HasColumnType("nvarchar(200)");

                    b.HasKey("Id");

                    b.HasIndex("ClienteId", "FechaProgramada");

                    b.HasIndex("GalponId", "Estado");

                    b.ToTable("tareas_vacunacion", "gestion_avicola", t =>
                        {
                            t.HasCheckConstraint("CK_tareas_vacunacion_aves", "[AvesVacunadas] IS NULL OR [AvesVacunadas] > 0");

                            t.HasCheckConstraint("CK_tareas_vacunacion_edad", "[EdadDia] > 0");

                            t.HasCheckConstraint("CK_tareas_vacunacion_estado_fecha", "[Estado] <> 'Completada' OR [FechaAplicacion] IS NOT NULL");
                        });
                });

            modelBuilder.Entity("Icarus.GestionAvicola.Domain.TransicionDespachoHuevo", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier");

                    b.Property<Guid>("ActorId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<Guid>("DespachoHuevoId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<int>("Destino")
                        .HasColumnType("int");

                    b.Property<DateTime>("FechaUtc")
                        .HasColumnType("datetime2");

                    b.Property<int>("Origen")
                        .HasColumnType("int");

                    b.HasKey("Id");

                    b.HasIndex("DespachoHuevoId");

                    b.ToTable("transiciones_despacho_huevo", "gestion_avicola");
                });

            modelBuilder.Entity("Icarus.GestionAvicola.Domain.TransicionPedidoAlimento", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier");

                    b.Property<Guid>("ActorId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<int>("EstadoDestino")
                        .HasColumnType("int");

                    b.Property<int>("EstadoOrigen")
                        .HasColumnType("int");

                    b.Property<DateOnly?>("FechaEntregaEstimada")
                        .HasColumnType("date");

                    b.Property<DateTime>("FechaUtc")
                        .HasColumnType("datetime2");

                    b.Property<string>("Motivo")
                        .HasMaxLength(500)
                        .HasColumnType("nvarchar(500)");

                    b.Property<Guid>("PedidoAlimentoId")
                        .HasColumnType("uniqueidentifier");

                    b.HasKey("Id");

                    b.HasIndex("PedidoAlimentoId");

                    b.ToTable("transiciones_pedidos_alimentos", "gestion_avicola");
                });

            modelBuilder.Entity("Icarus.GestionAvicola.Domain.DetalleDespachoHuevo", b =>
                {
                    b.HasOne("Icarus.GestionAvicola.Domain.DespachoHuevo", null)
                        .WithMany("Detalles")
                        .HasForeignKey("DespachoHuevoId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("Icarus.GestionAvicola.Domain.DetalleEntregaPedidoAlimento", b =>
                {
                    b.HasOne("Icarus.GestionAvicola.Domain.EntregaPedidoAlimento", null)
                        .WithMany("Lineas")
                        .HasForeignKey("EntregaPedidoAlimentoId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("Icarus.GestionAvicola.Domain.DetallePedidoAlimento", b =>
                {
                    b.HasOne("Icarus.GestionAvicola.Domain.PedidoAlimento", null)
                        .WithMany("Detalles")
                        .HasForeignKey("PedidoAlimentoId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("Icarus.GestionAvicola.Domain.DetallePrecioAlimento", b =>
                {
                    b.HasOne("Icarus.GestionAvicola.Domain.NotificacionPreciosAlimentos", null)
                        .WithMany("Detalles")
                        .HasForeignKey("NotificacionPreciosAlimentosId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("Icarus.GestionAvicola.Domain.DetallePrecioHuevo", b =>
                {
                    b.HasOne("Icarus.GestionAvicola.Domain.PublicacionPrecioHuevo", null)
                        .WithMany("Detalles")
                        .HasForeignKey("PublicacionPrecioHuevoId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("Icarus.GestionAvicola.Domain.DetalleRecepcionPedidoAlimento", b =>
                {
                    b.HasOne("Icarus.GestionAvicola.Domain.RecepcionPedidoAlimento", null)
                        .WithMany("Lineas")
                        .HasForeignKey("RecepcionPedidoAlimentoId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("Icarus.GestionAvicola.Domain.DocumentoDespachoHuevo", b =>
                {
                    b.HasOne("Icarus.GestionAvicola.Domain.DespachoHuevo", null)
                        .WithOne("DocumentoNota")
                        .HasForeignKey("Icarus.GestionAvicola.Domain.DocumentoDespachoHuevo", "DespachoHuevoId");
                });

            modelBuilder.Entity("Icarus.GestionAvicola.Domain.DocumentoNotaEntrega", b =>
                {
                    b.HasOne("Icarus.GestionAvicola.Domain.EntregaPedidoAlimento", null)
                        .WithMany("Documentos")
                        .HasForeignKey("EntregaPedidoAlimentoId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("Icarus.GestionAvicola.Domain.EntregaPedidoAlimento", b =>
                {
                    b.HasOne("Icarus.GestionAvicola.Domain.PedidoAlimento", null)
                        .WithOne("Entrega")
                        .HasForeignKey("Icarus.GestionAvicola.Domain.EntregaPedidoAlimento", "PedidoAlimentoId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("Icarus.GestionAvicola.Domain.ItemPlanVacunacion", b =>
                {
                    b.HasOne("Icarus.GestionAvicola.Domain.ProgramaVacunacion", null)
                        .WithMany("Items")
                        .HasForeignKey("ProgramaVacunacionId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("Icarus.GestionAvicola.Domain.RecepcionPedidoAlimento", b =>
                {
                    b.HasOne("Icarus.GestionAvicola.Domain.PedidoAlimento", null)
                        .WithOne("Recepcion")
                        .HasForeignKey("Icarus.GestionAvicola.Domain.RecepcionPedidoAlimento", "PedidoAlimentoId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("Icarus.GestionAvicola.Domain.TransicionDespachoHuevo", b =>
                {
                    b.HasOne("Icarus.GestionAvicola.Domain.DespachoHuevo", null)
                        .WithMany("Historial")
                        .HasForeignKey("DespachoHuevoId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("Icarus.GestionAvicola.Domain.TransicionPedidoAlimento", b =>
                {
                    b.HasOne("Icarus.GestionAvicola.Domain.PedidoAlimento", null)
                        .WithMany("Historial")
                        .HasForeignKey("PedidoAlimentoId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("Icarus.GestionAvicola.Domain.DespachoHuevo", b =>
                {
                    b.Navigation("Detalles");

                    b.Navigation("DocumentoNota");

                    b.Navigation("Historial");
                });

            modelBuilder.Entity("Icarus.GestionAvicola.Domain.EntregaPedidoAlimento", b =>
                {
                    b.Navigation("Documentos");

                    b.Navigation("Lineas");
                });

            modelBuilder.Entity("Icarus.GestionAvicola.Domain.NotificacionPreciosAlimentos", b =>
                {
                    b.Navigation("Detalles");
                });

            modelBuilder.Entity("Icarus.GestionAvicola.Domain.PedidoAlimento", b =>
                {
                    b.Navigation("Detalles");

                    b.Navigation("Entrega");

                    b.Navigation("Historial");

                    b.Navigation("Recepcion");
                });

            modelBuilder.Entity("Icarus.GestionAvicola.Domain.ProgramaVacunacion", b =>
                {
                    b.Navigation("Items");
                });

            modelBuilder.Entity("Icarus.GestionAvicola.Domain.PublicacionPrecioHuevo", b =>
                {
                    b.Navigation("Detalles");
                });

            modelBuilder.Entity("Icarus.GestionAvicola.Domain.RecepcionPedidoAlimento", b =>
                {
                    b.Navigation("Lineas");
                });
#pragma warning restore 612, 618
        }
    }
}
