...
public class GestionAvicolaEndpointsTests
{
    ...
    private async Task<string> LoginComo(string email)
    {
        ...
        var respuesta = await cliente.PostAsJsonAsync("/identidad/sesion", new { email, contrasena = IdentityFactory.ContrasenaDePrueba });
    ...
    private async Task<string> CrearClienteAvicola()
    {
        ...
        var modulos = await cliente.SendAsync(Autenticado(HttpMethod.Put, $"/clientes/{id}/modulos", admin, new { modulos = new[] { "GestionAvicola" } }));
    ...
    public async Task ClienteSemillaListaSuGranjaYGalpones()
    {
        ...
        using var cliente = _factory.CreateClient();
        var granjas = await cliente.SendAsync(Autenticado(HttpMethod.Get, "/granjas", token));
        ...
        var galpones = await cliente.SendAsync(Autenticado(HttpMethod.Get, $"/granjas/{id}/galpones", token));
    ...
    public async Task ClienteCreaUnaGranjaYLaSegundaDevuelve409()
    {
        ...
        using var cliente = _factory.CreateClient();
        var primera = await cliente.SendAsync(Autenticado(HttpMethod.Post, "/granjas", token, new { nombre = $"Granja {Guid.NewGuid():N}" }));
        Assert.Equal(HttpStatusCode.Created, primera.StatusCode);
        var segunda = await cliente.SendAsync(Autenticado(HttpMethod.Post, "/granjas", token, new { nombre = $"Otra {Guid.NewGuid():N}" }));
    ...
    public async Task IdDeOtroTenantDevuelve404()
    {
        ...
        using var cliente = _factory.CreateClient();
        var alta = await cliente.SendAsync(Autenticado(HttpMethod.Post, "/granjas", clienteToken, new { nombre = "Granja Tenant A" }));
        var id = (await alta.Content.ReadFromJsonAsync<JsonElement>()).GetProperty("id").GetGuid();
        var otro = await cliente.SendAsync(Autenticado(HttpMethod.Get, $"/granjas/{id}", otroClienteToken));
    ...
    public async Task GalponConFechaFuturaDevuelve400()
    {
        ...
        var respuesta = await cliente.SendAsync(Autenticado(HttpMethod.Post, $"/granjas/{id}/galpones", token, new { numero = $"F{Guid.NewGuid():N}"[..10], capacidadMaxima = 100, gallinasActuales = 0, fechaNacimientoLote = fecha }));
